...
[[fig-UserProfileCard]]
.After you have provided cthe appropriate properties in your profile.
image::userprofilecard.png[width="450"]
{nbsp} +
Type `recommend` in the command box, press kbd:[Enter] and you have <<fig-NoPrefixRecommend>>. +
{nbsp} +
[[fig-NoPrefixRecommend]]
.Recommended result with exactly the same properties as the workout presets in <<fig-UserProfileCard>>
