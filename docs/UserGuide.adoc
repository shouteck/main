...
* You can change the state tag from both `future` and `completed`. The current command will receive a warning when the indexed workout's difficulty is harder than the user's indicated difficulty,
* duration is higher than the user's indicated duration or calories is higher than the user's indicated calories.
****
