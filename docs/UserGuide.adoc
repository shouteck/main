...
****
* `SUBCOMMAND` can only be either `start` or `stop`. If it's `start`, WorkoutBook will start tracking the parameter, and for `stop`, WorkoutBook will stop the tracing.
