...
[NOTE]
====
When one workout is requested (either through the user's WorkoutPresets or user input with a specified `Mode` of single) and there is more than one matching workout, a random matching workout is retrieved.
====
