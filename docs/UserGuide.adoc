...
Format: `filter [type/TYPE] [duratiorn/DURATION] [equipment/EQUIPMENT]`
...
Returns any workout having duration of `20m`, equipment of `dumbbell`, type of `strength`
...
Recommend a workout from the existing workouts identified by the `parameter` and `PARAMETER`. +
Format: `recommend parameter/PARAMETER`

****
* `parameter` only accepts one of the three prefixes which is either `difficulty`, `calories` or `duration`.
* See Command Format on which `PARAMETER` is acceptable for its corresponding `parameter`.
****
...
* `recommend duration/10m` +
Recommends a 10 minute workout.
