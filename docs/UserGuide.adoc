...
* Duplicate workouts includes are as f
****
...
Recommends workout from the existing workouts. +
Format: `recommend [mode/MODE] [calories/CALORIES or ocalories/CALORIES] [difficulty/DIFFICULTY or odifficulty/ODIFFICULTY] [duration/DURATION or oduration/DURATION]`
...
* Mode is a compulsory prefix when you want to recommend with any combinations of the three prefixes from `calories`, `difficulty` and `duration` excluding zero combinations.
* Recommend single, multiple or all workouts depending on the `MODE`.
* If you did not use Optional prefixes which has an `o` in front of either of the three prefixes from `calories`, `difficulty` and `duration`, The workout recommended has
exactly the same inputs as the one you have entered E.g. `recommend mode/single difficulty/beginner` will only recommend the workout that have difficulty level as beginner.
* If you do use Optional prefixes, you have to supply all the three prefixes `calories`, `difficulty` and `duration` where they can be in their Optional form or not.
E.g `recommend mode/single ocalories/150 difficulty/beginner duration/10m` will recommend a workout with these three properties if it is present or the next best alternative
which is a 10 minute workout with a difficulty level of beginner.
****
...
* `recommend mode/single calories/250 difficult/intermediate oduration/10m` +
Recommends a 10 minute intermediate workout if it is present or else it will recommend an intermediate workout that burns 250 calories.
* `recommend mode/single calories/300 odifficulty/beginner oduration/20m` +
Recommends a 20 minute beginner workout that burns 300 calories if it is present or else it will recommend the next best alternative.
* `recommend mode/single ocalories/350 odifficulty/advanced oduration/30m` +
Recommends a 30 minute advanced workout that burns 350 calories if it is present or else it will recommend the next best alternative.
