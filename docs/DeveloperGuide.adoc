...
[[fig-RecommendCommandSD]]
.Interactions inside `Logic` component for the *`recommend difficulty/beginner`* command.
image::RecommendCommandSD.PNG[width="900"]

==== Design Considerations

==== Aspect: How recommend executes

* **Alternative 1 (current choice):** Filters the workout list based on user's input and randomly retrieves a workout from that filtered list.
** Pros: Easy to implement.
** Cons: May not reflect what the user actually wants.
* **Alternative 2:** Use machine learning algorithms.
** Pros: Able to recommend a workout that is more accurate to what the user wants.
** Cons: Complicated to implement. Requires a huge storage to store and record the substantial amount of data that comes from the interactions between the user and the application.
// end::recommend[]
...
image::CurrentCommandSD.png[width="500"]
...
image::CompleteCommandSD.png[width="500"]
...
// tag::trackfeature
==== Current Implementation
The user input is parsed into `_TrackCommandParser_` to generate a `Parameter` object, which is then passed on to `_TrackCommand_`. Upon execution of the command, the `Parameter` is then stored in/removed from the `TrackedDataList`. This `TrackedDataList` stores all the `Parameters` currently being tracked.

When a workout is marked as completed using the memory feature, the model will scan the `TrackedDataList` to check if any of the tracked `Parameters` are present in the completed workout. If present, they will be stored in `TrackedData`.
