...
==== Current implementation
There are several functionalities in the recommend command and to invoke certain functionalities, different combination
of inputs are considered.When the user keys in the input in the command box, the user input is parsed in
RecommendCommandParser.An instance of RecommendArguments class will be instantiated corresponding to the user input and
this instance is then passed to RecommendCommand in the Logic component.

In order to recommend a suitable workout for a given combination of inputs which are stored in the RecommendArguments
instance, we must first find out what are the suitable workouts present in our current workout list.

The instance of RecommendArguments is passed through a getFinalFilteredInternalList() method in the Model component.
This method works by filtering the current workout list to match the user inputs as much as possible to return a
filtered list instead of mutating the original copy.

Here, we introduce the notion of optional prefixes where users are allowed to indicate whether certain properties are
mandatory or good to have to have in their workouts. By allowing users the flexibility to decide the mandatory
properties they want in a workout, we can cut down the number of steps taken by a user to arrive at their optimal
recommended workout. The worst case scenario of finding an optimal workout takes approximately 2^(n) steps,
where n is the number of properties which is 3 in this case.

There are different levels in what workouts are to be recommended and considered. The algorithm ensures that the workout
recommended will be of the highest level or the greatest match.
// end::recommend[]
