...
When the user keys in the input in the command box, the user input is parsed in the `RecommendCommandParser`.
...
Here, we introduce the notion of optional prefixes where users are allowed to indicate whether certain properties are mandatory or good to have in their workouts.
