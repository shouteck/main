...
[[fig-ClickProfile]]
.Interactions inside `UI` component when clicking profile
...
[[fig-F2Profile]]
.Interactions inside `UI` component when pressing `F2`
...
* `SearchCommand()` - Search the workout by partial keywords
...
image::CurrentCommandSD.png[width="700"]
...
image::CompleteCommandSD.png[width="700"]

The user can consider changing the state tag directly without transitioning through `current` i.e. from `future` to `completed` by using the complete command.
The user can also reverse the state tag i.e. from `completed` to `current` by using the current command.
