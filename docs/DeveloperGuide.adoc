...
After executing the command `_profile_`, it will create an instance which uses `showProfileRequestEvent` method of `Events`. Then, the `UI` will return `handleProfile` and the profile window will show up.
