...
// tag::profile
==== Current implementation
The profile window is first being loading by loading the `_profileWindow.fxml file_`. Thereafter, once the user request to open the profile window, it will then process it.
...
After executing the command `_profile_`, it will create an instance which uses `showProfileRequestEvent` method of `Events`. Then, the `UI` will `handleShowProfileRequestEvent` and then return `handleProfile` and the profile window will show up.
