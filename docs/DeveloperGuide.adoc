...
+
[WARNING]
You must include this link:https://github.com/sijieg/main/blob/master/ProfileWindow.html[`ProfileWindow.html`] file in the same location as your `workoutbook.jar` file or else you will not be able to run the `workoutbook.jar`
...
==== Current implementation
There are several functionalities in the recommend command and to invoke certain functionalities, different combinations of inputs are considered.
When the user keys in the input in the command box, the user input is parsed in `RecommendCommandParser`.
An instance of `RecommendArguments` class will be instantiated corresponding to the user input and this instance is then passed to `RecommendCommand` in the `Logic` component.

In order to recommend a suitable workout for a given combination of inputs which are stored in the `RecommendArguments` instance, we must first find out what are the suitable workouts present in our current workout list.
The instance of `RecommendArguments` is passed through a `getFinalFilteredInternalList()` method in the `Model` component.
This method works by filtering the current workout list to match the user inputs as much as possible to return a filtered list instead of mutating the original copy.

Here, we introduce the notion of optional prefixes where users are allowed to indicate whether certain properties are mandatory or good to have to have in their workouts.
By allowing users the flexibility to decide the mandatory properties they want in a workout, we can cut down the number of steps taken by a user to arrive at their optimal recommended workout.
The worst case scenario of finding an optimal workout takes approximately 2^(n) steps, where n is the number of properties which is 3 in this case.
There are different levels in what workouts are to be recommended and considered. The algorithm ensures that the workout recommended will be of the highest level or the greatest match.

There are two different behavioral sequences that will arise under certain combinations of inputs. The activity diagram below depicts the execution flow of the `recommend` command.
...
Shown below is the component interactions for the `recommend` command where the path taken is with the condition of recommending directly from Workout Presets in the user's profile or through the inputs with a specified mode of single.

[[fig-RecommendEventSD]]
.Component interactions for `recommend` command (part 1).
image::SDForRecommendEvent.png[width="700"]
{nbsp} +

The event then propagates through `EventsCenter` to the `UI`. The `UI` will call `handleJumpToRecommendListRequestEvent()` to handle the event, which eventually results in the the `UI` being updated to scroll to the workout in the `WorkoutListPanel` and selecting it.
The process is shown in the Figure below.

[[fig-RecommendEventHandlingSD]]
.Component interactions for `recommend` command (part 2).
...
----
`WorkoutsPredicate` instance is then passed to `updateFilteredWorkoutList()` method of `Model` to change the entire list of workouts displayed to those that matched at least one of the workouts.
...
* **Alternative 2:** Recommend based on user's interactions with the system.
** Pros: The optimal workout will be recommended immediately without supplying any inputs.
...
// end::recommend-considerations[]

=== User Profile
// tag::profile
==== Current implementation
The profile window is first being loaded by loading the `_profileWindow.fxml file_`. Thereafter, once the user request to open the profile window, it will then process it.

There are 3 ways to open a profile window.
...
After executing the command `profile`, it will create an instance which uses `showProfileRequestEvent` method of `Events`. Then, the `UI` will `handleShowProfileRequestEvent` and the profile window will be opened.
...
* `FilterCommand()` -Filters the workout book

* `SelectCommand()` -Select the workout by index number

* `RemarkCommand()` -Remark the workout by index number

Given below is an example usage scenario and how the traversing mechanism behaves at each step.

Step 1. The user launches the application, `add` some workouts to the workout book, then `sort` the workout.
The sort command calls the `sortFilteredWorkoutList()` of `Model` to sort the workout based on the names in alphabetical order.

Step 2. The user executes `filter` to find the workouts that contain `duration`, `equipment` and `type`.

Step 3. The user executes `remark` to remark a workout

image::traversingSequenceDiagram.png[width="650"]

image::HighLevelRemarkSequenceDiagrams.png[width="650"]
...
image::CurrentCommandSD.png[width="800"]
...
image::CompleteCommandSD.png[width="800"]
...
// tag::trackfeature[]
==== Current Implementation
The user input is parsed into `_TrackCommandParser_` to generate a `Parameter` object, which is then passed on to `_TrackCommand_`. Upon execution of the command, the `Parameter` is then stored in/removed from the `TrackedDataList`. This `TrackedDataList` stores all the `Parameter` objects currently being tracked.
