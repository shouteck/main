...
Allows the user to find workouts that have the respective `Calories`, `Duration` and `Difficulty`.
Allows the user to find workouts that have the respective `Calories`, `Duration` and `Difficulty` under the workout presets in the user profile.
Allows the user the flexibility to decide whether any of these `Calories`, `Duration` and `Difficulty` is mandatory or good to have when finding workouts.
...
** [.underline]#Highlights#: This feature required a in-depth analysis of design alternatives where I implemented a builder pattern to accommodate the vast amount of different combinations of constructors that have to called. Moreover, I have to decide on a meaningful way of expressing user inputs on the CLI so that it is intuitive to the user.
...
include::../UserGuide.adoc[tag=commandformat,tag=recommend]
