...
import static seedu.address.commons.core.Messages.MESSAGE_INVALID_PARAMETER_DISPLAYED_INDEX;
import static seedu.address.commons.core.Messages.MESSAGE_UNKNOWN_COMMAND;
import static seedu.address.logic.commands.SelectCommand.MESSAGE_SELECT_PARAMETER_SUCCESS;
import static seedu.address.testutil.TestUtil.getLastParameterIndex;
import static seedu.address.testutil.TestUtil.getMidParameterIndex;
...
public class SelectCommandSystemTest extends WorkoutBookSystemTest {
    ...
    public void select() {
        ...
        /* Case: select the last card in the tracked data list -> selected */
        Index parameterCount = getLastParameterIndex(getModel());
        command = SelectCommand.COMMAND_WORD + " " + parameterCount.getOneBased();
        assertCommandSuccess(command, parameterCount);
        ...
        /* Case: select the middle card in the parameter list -> selected */
        Index middleIndex = getMidParameterIndex(getModel());
        ...
        int invalidIndex = getModel().getFilteredTrackedDataList().size() + 1;
        assertCommandFailure(SelectCommand.COMMAND_WORD + " " + invalidIndex,
                MESSAGE_INVALID_PARAMETER_DISPLAYED_INDEX);
     ...
     * {@code expectedSelectedCardIndex} of the selected parameter.<br>
     * 4. {@code Storage} and {@code TrackedDataListPanel} remain unchanged.<br>
